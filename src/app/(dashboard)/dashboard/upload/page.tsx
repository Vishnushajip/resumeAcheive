...
export default function UploadPage() {
  ...
  const [status, setStatus] = useState<"idle" | "extracting" | "parsing" | "done">("idle");
    ...
    try {
      ...
      try {
        await cvService.createCV({
          title: aiResponse.resume?.personalInfo?.fullName + " - Extracted" || "Extracted CV",
  ...
  return (
    <div className="container mx-auto px-4 py-16 max-w-4xl min-h-[calc(100vh-80px)] font-sans">
      <div className="text-center space-y-4 mb-20 animate-fade-in">
        <h1 className="text-5xl md:text-6xl font-black font-outfit tracking-tighter">Upload <span className="text-primary">Existing</span> CV</h1>
        <p className="text-xl text-zinc-500 max-w-2xl mx-auto font-medium">
          Our client-side engine extracts text and rebuilds your resume using advanced AI models.
        </p>
      </div>
        ...
        {file ? (
          <div className="space-y-10 animate-in zoom-in-95 duration-500">
            <div className="relative inline-block">
              {preview && (
                <img src={preview} alt="CV Preview" className="h-64 w-48 object-cover rounded-3xl shadow-2xl border-4 border-white mx-auto transition-transform group-hover:scale-105" />
              )}
              <Button 
                variant="destructive" 
                size="sm" 
            ...
            <div className="space-y-2">
              ...
              <p className="text-zinc-500 font-bold uppercase tracking-widest text-xs">{(file.size / 1024 / 1024).toFixed(2)} MB • READY TO ANALYZE</p>
            </div>
            
            {status === "idle" ? (
              <Button size="lg" className="w-full h-20 text-xl font-black rounded-[2rem] bg-primary shadow-xl shadow-primary/30 uppercase tracking-widest" onClick={processImage}>
                Analyze & Rebuild
              </Button>
            ) : (
              <div className="space-y-6 pt-4 max-w-md mx-auto">
                ...
                <div className="flex items-center justify-center gap-4 text-primary font-black text-lg font-outfit uppercase tracking-tighter">
                  {status === "extracting" && <><Loader2 className="h-6 w-6 animate-spin" /> Extracting Text ({progress}%)</>}
                  {status === "parsing" && <><Sparkles className="h-6 w-6 animate-pulse" /> AI Structuring Content...</>}
                  {status === "done" && <><CheckCircle2 className="h-6 w-6" /> Success! Redirecting...</>}
                </div>
              </div>
            )}
          </div>
          ...
          <label className="cursor-pointer">
            ...
            <div className="flex flex-col items-center gap-8 py-20">
              ...
              <div className="space-y-3">
                <p className="text-3xl font-black font-outfit tracking-tight">Drop your CV here</p>
                <p className="text-zinc-500 font-medium text-lg italic">Supports HD Images (PNG, JPG) and PDF Documents</p>
              </div>
              <Button variant="outline" className="pointer-events-none px-10 h-14 rounded-2xl border-2 font-bold uppercase tracking-widest">Select From Folder</Button>
            </div>
          </label>
        )}
      </div>
      ...
      <div className="mt-24 grid grid-cols-1 md:grid-cols-2 gap-10">
        <div className="flex gap-6 p-10 bg-white border border-zinc-100 rounded-[3rem] shadow-sm hover:shadow-xl transition-shadow">
          ...
          <div>
            <h3 className="text-2xl font-black font-outfit mb-3">Privacy First</h3>
            <p className="text-zinc-500 font-medium leading-relaxed">OCR runs entirely in your browser. Your documents never hit our servers during extraction.</p>
          </div>
        </div>
        <div className="flex gap-6 p-10 bg-white border border-zinc-100 rounded-[3rem] shadow-sm hover:shadow-xl transition-shadow">
          ...
          <div>
            <h3 className="text-2xl font-black font-outfit mb-3">Instant Capture</h3>
            <p className="text-zinc-500 font-medium leading-relaxed">Our AI recognizes professional experience, education, and skills with 99% accuracy across industries.</p>
          </div>
        </div>
      </div>
    </div>
